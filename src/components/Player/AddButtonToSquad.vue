<template>
    <button v-if="!isPlayerInTeam" class="add_playerToSquad" type="button" @click="openModal">
...
import { addPlayerToTeam } from '@/api/player';
...
const isPlayerInTeam = computed(() => {
    return props.player.teams && props.player.teams.length > 0; 
});
...
const selectTeam = async (team) => {
    try {
        await addPlayerToTeam(props.player.id, team.id);
        emit('team-selected', { team: { ...team, teamName: team.teamName }, player: props.player });
        closeModal();
    } catch (error) {
